...
@pytest.mark.parametrize("traj_path", TRAJ_PATHS, ids=[str(path) for path in TRAJ_PATHS])
def test_traj_deserialize_serialize_preserves_stored_values(traj_path):
    dataset = Dataset(traj_path)
    stored = dataset._read_binary_file(traj_path, dataset.numpy_dtype, dataset.shape_storage)

    serialized = dataset._schema.serialize(dataset.data, dataset._schema.layouts)

    assert serialized.shape == dataset.shape_storage
    assert (serialized == stored).all()
